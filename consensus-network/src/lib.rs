...
pub struct NetworkHandle {
    incoming_rx: mpsc::UnboundedReceiver<proto::Envelope>,
    publish_tx: mpsc::UnboundedSender<proto::Envelope>,
    pub listen_addr: String,
}

impl NetworkHandle {
    pub async fn incoming(&mut self) -> Option<proto::Envelope> {
        self.incoming_rx.recv().await
    }

    /// Publish envelope to peers (async fire-and-forget)
    pub async fn publish(&self, env: proto::Envelope) -> Result<(), Box<dyn Error + Send + Sync>> {
...
pub async fn start(listen_addr: &str, peers: Vec<String>) -> Result<NetworkHandle, Box<dyn Error + Send + Sync>> {
    let (incoming_tx, incoming_rx) = mpsc::unbounded_channel::<proto::Envelope>();
    let (publish_tx, mut publish_rx) = mpsc::unbounded_channel::<proto::Envelope>();
                ...
                Ok((socket, peer_addr)) => {
                    ...
                    tokio::spawn(async move {
                        ...
                        loop {
                            ...
                            match proto::Envelope::decode(&*data) {
                                Ok(env) => {
                                    let _ = tx.send(env);
                                }
                                Err(e) => log::warn!("Failed to decode Envelope from {}: {}", peer_addr, e),
                            }
                        }
                    });
                }
